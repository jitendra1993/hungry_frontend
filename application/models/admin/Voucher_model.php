...
class Voucher_model extends CI_Model{
	...
	public function getCountVoucher($filter){
		...
		if(!empty($filter))
		{
			if(!empty($filter['filter_name']))
			{
				...
				$name[5]['voucher_name'] = array('$regex'=>$filter_name,'$options'=>'i');
				$match['$match']['$or']=$name;
		...
		$results = $this->db->master_voucher->aggregate($ops);
		$total = 0;
	...
	public function getMasterVouchers($filter,$data = array()){
		...
		$ops = array(
			...
			$match,
			array('$project' =>['_id'=>0,'id'=>1,'voucher_name'=>1,'voucher_type'=>1,'delivery'=>1,'pickup'=>1,'dinein'=>1,'voucher_price'=>1,'voucher_min_order'=>2,'valid_from'=>1,'valid_to'=>1,'status'=>1,'added_date_timestamp'=>1,'updated_date_timestamp'=>1,'user_hash_id'=>1,'m.merchant_name'=>1,'m.merchant_phone'=>1,'m.contact_name'=>1,'m.contact_phone'=>1,'m.contact_email'=>1]),
	...
	public function getVoucherById($id){
		$cond = array('id'=> $id);
		$admin_info = $this->db->master_voucher->findOne($cond);
		return (object) $admin_info;
	}
	
	public function deleteVoucher( $data,$id){
		$cond = array('id'=>$id);
		$this->db->master_voucher->updateMany($cond,array('$set'=>$data),array("multi"=>false));
		return true;
	}
	...
	public function addVoucher($data){
		$this->db->master_voucher->insertOne($data);
		return true;
	}

<?php
class Addonitem_model extends CI_Model{
	...
	public function getCountAddonItem($filter){
		...
		$ops = array(
			...
			$match,
			array('$project' =>['_id'=>0,'id'=>1,'name'=>1]),
			array('$group' => array('_id' => null,'count' => array('$sum' => 1)))
		);
		
		$results = $this->db->master_addon_item->aggregate($ops);
		$total = 0;
	...
	public function getAddonItem($filter,$data = array()){
		...
		$ops = array(
			...
			$match,
			array('$project' =>['_id'=>0,'id'=>1,'name'=>1,'status'=>1,'added_date_timestamp'=>1,'updated_date_timestamp'=>1,'user_hash_id'=>1,'image'=>1,'addon_categories'=>1,'price'=>1,'m.merchant_name'=>1,'m.merchant_phone'=>1,'m.contact_name'=>1,'m.contact_phone'=>1,'m.contact_email'=>1]),
	...
	public function getAddonItemById($id){
		$cond = array('id'=> $id);
		$admin_info = $this->db->master_addon_item->findOne($cond);
		return (object) $admin_info;
	}
	
	public function updateAddonItem($data,$id){
		$cond = array('id'=>$id);
		$this->db->master_addon_item->updateMany($cond,array('$set'=>$data),array("multi"=>false));
		return true;
	}
	
	public function addAddonItem($data){
		$this->db->master_addon_item->insertOne($data);
		return true;
	}
	
	public function deleteAddAddonItem( $data,$id){
		$cond = array('id'=>$id);
		$this->db->master_addon_item->updateMany($cond,array('$set'=>$data),array("multi"=>false));
		return true;
	}
	
	public function getAddOnCategoryArray(){
		$user_id = $this->session->userdata('user_id');
		...
		$results = $this->db->master_addon_category->find($cond,['sort' => ['name' => -1]]);
		$arr =[];

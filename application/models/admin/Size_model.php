...
class Size_model extends CI_Model{
	...
	public function getCountSize($filter){
		...
		$results = $this->db->master_size->aggregate($ops);
		$total = 0;
	...
	public function getMasterSize($filter,$data = array()){
		$match = [];
		$match['$match']['status']=array('$nin'=>array(2));
		if ($this->session->userdata('role_master_tbl_id')==2) {
			$user_id = $this->session->userdata('user_id');
	...
	public function addSize($data){
		$this->db->master_size->insertOne($data);
		return true;
	}
	...
	public function getSizeById($id){
		...
		$cond = array('id'=> $id);
		$admin_info = $this->db->master_size->findOne($cond);
	...
	public function updateSize($data,$id){
		...
		$cond = array('id'=>$id);
		$this->db->master_size->updateMany($cond,array('$set'=>$data),array("multi"=>false));
		return true;
	}
	...
	public function getAllSizeArray(){
		$user_id = $this->session->userdata('user_id');
		...
		$results = $this->db->master_size->find($cond,['sort' => ['name' => -1]]);
		$arr =[];

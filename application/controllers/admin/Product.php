	...
	public function productAdd($id=''){
		...
		if($this->input->method(TRUE) == 'POST'){
			...
			if($meal_deal!=1){
				...
				$this->form_validation->set_rules('item_price', 'Product Price', 'trim|required');
			}
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$multi_option_value_arr = $this->input->post('multi_option_value[]');
				$two_flavors_position = $this->input->post('two_flavors_position[]');
				$require_addon = (!empty($this->input->post('require_addon[]')))?$this->input->post('require_addon[]'):array();
				...
				// print_r($multi_option_value_arr);
				...
				if(isset($addon_item_id) && count($addon_item_id)>0){
					$addon_item =[];
					foreach($addon_item_id as $category_id=>$addonitems){

						if(count($addonitems)>0){
							$multi_option_value = 1;
							if($multi_option[$category_id]=='1'){
								$multi_option_value = 1;

							}else if($multi_option[$category_id]=='2'){
								$multi_option_value = 2;

							}else if($multi_option[$category_id]=='3'){
								$multi_option_value = $multi_option_value_arr[$category_id];
							}
						
							$addon_item['category_id'] = $category_id;
							$addon_item['addon_item'] = array_keys($addonitems);
							$addon_item['multi_option'] = $multi_option[$category_id];
							$addon_item['multi_option_value'] = $multi_option_value;
							$addon_item['two_flavors_position'] = $two_flavors_position[$category_id];
							$addon_item['require_addon'] = !empty($require_addon[$category_id])?$require_addon[$category_id]:'0';
							$addon_item['meal_deal_no_option'] = !empty($meal_deal_no_option[$category_id])?$meal_deal_no_option[$category_id]:'0';
							$addon_item['meal_deal_item_qty'] = !empty($meal_deal_item_qty[$category_id])?$meal_deal_item_qty[$category_id]:(Object)[];

							$addon_item_arr[] = $addon_item;
						}
					}
				}
				...
				$product_data = array(
					...
					'item_price' => (!empty($this->input->post('item_price')))?$this->input->post('item_price'):0,
